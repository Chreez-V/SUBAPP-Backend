...
export async function pagarNfcController(
  ...
  try {
    ...
    const route: any = await Route.findById(routeId).session(session);
    if (!route) {
      throw new Error("ROUTE_NOT_FOUND");
    }
    // 2. Validar Tarjeta NFC (Tratamos el modelo como 'any')
    const nfcCard: any = await (NfcCard as any)
      .findOne({ cardUid })
      .session(session);
    ...
    const busFare: any = await BusFare.findOne({}).session(session);
    ...
    const discount: any = await (DiscountProfile as any)
      .findOne({
        userId: user._id,
        status: "aprobado",
      })
