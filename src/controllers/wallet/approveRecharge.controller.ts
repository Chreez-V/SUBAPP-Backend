 ...
 *
 * Aprobamos una recarga de saldo previamente validada
 *
 * @param {FastifyRequest<{ Params: PaymentParams }>} request
...
export async function approveRechargeController(
  ...
  try {
    ...
    if (paymentValidation.status !== 'pendiente') {
      return reply
        .code(400)
        .send({ message: `The payment is not awaiting approval.` })
    }
